...
app.get("/pokemonNames", async function(req, res) {
  try {
    ...
    var sufix_image = ".png";
    axios
      .get("https://pokeapi.co/api/v2/pokemon/?limit=" + MAXPOKEMON)
      .then(function(response) {
...
function getRangeOfPokemon(start, end) {
  ...
  function request() {
    if (data[start - 1] && !dataInCache) {
      ...
      console.log(
        "2 " + "d.length = " + d.length + " begin= " + begin + " end=" + end
      );
...
app.get("/getByIDs", async function(req, res) {
  ...
  try {
    ...
    if (query.startID && query.endID) {
      var pokemonBatch = await getRangeOfPokemon(
        parseInt(query.startID),
        parseInt(query.endID)
      );
